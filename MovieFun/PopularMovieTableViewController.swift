...
class PopularMovieTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = "大家都愛看"
        if let urlStr = "https://api.themoviedb.org/3/movie/popular?api_key=6ac4780523b1635d3bf190ca8b4658ce&language=zh-EN&page=1".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed), let url = URL(string: urlStr) {
        ...
        if let urlStr = "https://api.themoviedb.org/3/movie/popular?api_key=dfe7dcf94059005500c15e5ca070550f&language=zh-EN&page=2".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed), let url = URL(string: urlStr) {
            //weak var weakSelf = self
            let task = URLSession.shared.dataTask(with: url) { (data, response , error) in
